    ...
    <div style={{ maxWidth: "1400px", margin: "0 auto" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "2rem" }}>
        <div>
          ...
          <p style={{ color: "var(--color-slate-500)", fontSize: "0.95rem" }}>Manage your working hours and block out dates.</p>
        </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gap: "1.5rem" }}>
        ...
        <div style={{ backgroundColor: "var(--color-white)", borderRadius: "0.75rem", border: "1px solid #e2e8f0", padding: "1.5rem", boxShadow: "0 1px 3px rgba(0,0,0,0.05)" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
            {days.map((day, i) => (
              <div key={day} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "1rem", backgroundColor: "var(--color-slate-50)", borderRadius: "0.5rem", border: "1px solid #e2e8f0" }}>
                <div style={{ display: "flex", alignItems: "center", gap: "1rem", width: "150px" }}>
                  ...
                  <span style={{ fontWeight: "600", color: "var(--color-slate-900)", fontSize: "0.95rem" }}>{day}</span>
                </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem" }}>
          <div style={{ backgroundColor: "var(--color-white)", borderRadius: "0.75rem", border: "1px solid #e2e8f0", padding: "1.5rem", boxShadow: "0 1px 3px rgba(0,0,0,0.05)" }}>
            ...
            <p style={{ fontSize: "0.85rem", color: "var(--color-slate-500)", marginBottom: "1rem" }}>Add specific dates when you are unavailable for bookings (e.g. holidays).</p>
            
            <div style={{ display: "flex", gap: "0.5rem", marginBottom: "1.5rem" }}>
              <input type="date" style={{ flex: 1, padding: "0.5rem", borderRadius: "0.5rem", border: "1px solid #cbd5e1", outline: "none" }} />
              <button style={{ padding: "0.5rem 1rem", backgroundColor: "var(--color-slate-900)", color: "var(--color-white)", border: "none", borderRadius: "0.5rem", fontWeight: "500", cursor: "pointer" }}>Add</button>
            </div>

            <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
              {["Sep 10, 2025 - Sep 14, 2025 (Vacation)", "Nov 27, 2025 (Holiday)"].map((date, idx) => (
                <div key={idx} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "0.75rem 1rem", backgroundColor: "var(--color-amber-50)", borderRadius: "0.5rem", border: "1px solid #fde68a", fontSize: "0.85rem", color: "var(--color-amber-700)" }}>
                  <span>{date}</span>
                  <button style={{ background: "none", border: "none", color: "var(--color-amber-700)", cursor: "pointer" }}><X size={14} /></button>
                </div>
              ))}
            </div>
          </div>
